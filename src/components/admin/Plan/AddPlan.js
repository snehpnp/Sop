...
import { ChartingPlatformSegment } from '../../CommonAPI/User';
...
const AddPlanPage = () => {
    ...
    const [selecteGoldenStrategy, setSelecteGoldenStrategy] = useState([]);
    ...
    const formik = useFormik({
        ...
        validate: (values) => {
            ...
            // AddOn validation for Price and NumberofScript
            if (formik.values.PlanType === "AddOn") {
                if (!values.Price || values.Price === 0) {
                    errors.Price = "Please enter the price.";
                }
                if (!values.NumberofScript || values.NumberofScript === 0) {
            ...
            // Only validate SOPPrice and planname if PlanType is not AddOn
            if (formik.values.PlanType !== "AddOn") {
                if (!values.SOPPrice) {
        ...
        onSubmit: async (values) => {
            if (formik.values.PlanType == "Scalping" && selecteScalping.length === 0 && selecteOptions.length === 0 && selectePattern.length === 0 && selecteGoldenStrategy.length === 0) {
                showError("Error!", "Please select at least one strategy either Scalping, Option , Pattern or Golden Strategy.");
                return;
            }
            ...
            const req = {
                ...
                GoldenStrategy: selecteGoldenStrategy.map((strategy) => strategy.value),
            };
    ...
    const handleSelectChange = (type, selected) => {
        if (type === "scalping") {
            ...
            setSelectedCharting([]);
        } else if (type === "goldenstrategy") {
            setSelecteGoldenStrategy(selected);
            setSelectedCharting([]);
        }
    }; 
            ...
            <AddForm
                ...
                additional_field={
                    <>
                            ...
                            <div className="row" style={{ width: "100%" }}>
                                <div className="col-lg-12 dropdown-multi" style={{ width: '36vw' }}>
                                    ...
                                    <CustomMultiSelect
                                        ...
                                        options={chartingSegmentOption.map(segment => ({ value: segment, label: segment }))}
                        ...
                        {formik.values.PlanType == "Scalping" && adminPermission?.includes("Golden Strategy") && (
                            
                                <CustomMultiSelect
                                    label={<span className='card-text-Color mt-2'>Golden Strategy</span>}
                                    options={["MomentumPicker", "Jobbing"].map(strategy => ({ value: strategy, label: strategy }))}
                                    selected={selecteGoldenStrategy}
                                    onChange={(selected) => handleSelectChange("goldenstrategy", selected)}
                                />
