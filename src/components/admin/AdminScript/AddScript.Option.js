...
import { Add_Group, AddAdminScript, GET_EXPIRY_DATE } from '../../CommonAPI/Admin'
...
const AddClient = () => {
        ...
        validate: (values) => {
            ...
            if (values.Striketype == "Depth_of_Strike" && values.Measurment_Type != "Shifting/FourLeg" && values.Strategy != 'LongStraddle' && values.Strategy != 'ShortStraddle') {
                if (values.DepthofStrike > 10 || values.DepthofStrike < -10) {
                    errors.DepthofStrike = "Enter Depth of Strike Value Between -10 to 10.";
                }
            }
    ...
    return (
        <>
            <Content
                Page_title={`Add Script - Option Strategy  , Group Name : ${location.state.groupData.GroupName}`}
                button_status={false}
