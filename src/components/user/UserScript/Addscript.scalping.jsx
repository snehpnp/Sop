...
const AddClient = () => {
  ...
  const formik = useFormik({
    ...
    validate: (values) => {
      ...
      if (
        ...
        (values.RolloverTF == "" ||
          values.RolloverTF == undefined ||
          (values.RolloverTF == null &&
            values.Strategy == "Multi_Conditional" &&
            values.position_type == "Multiple"))
      ) {
        errors.RolloverTF = "Please Enter Rollover";
      }
      if (
  ...
  useEffect(() => {
    ...
    formik.setFieldValue("RolloverTF", location?.state?.data?.ExitDay == "Delivery" ? true : false);
    ...
    },
    {
      ...
      col_size:
        (formik.values.Instrument === "OPTFUT" ||
          ...
          formik.values.Exchange === "MCX"
          ? 3
          : (formik.values.Instrument === "IO" ||
            formik.values.Instrument === "SO") &&
            formik.values.Exchange === "BFO"
            ? 4 : formik.values.Instrument === "FUTSTK" ||
              formik.values.Instrument === "FUTIDX"
              ? 3 :
              4,
      disable: false,
