...
import DynamicButton from "../../../ExtraComponent/Button";
...
const Coptyscript = ({ tableType, data, selectedType, FromDate, ToDate }) => {
  ...
  const AddScript = (data) => {
    ...
    const lastIndex1 = allScripts?.len;
    const lastIndex2 = allScripts2?.len;
    ...
    switch (data) {
      case "ChartingPlatform":
        const chartingData =
          allScripts2?.data?.[lastIndex2]?.CombineChartingSignal;
        if (chartingData?.length >= 1) {
      ...
      case "Option Strategy":
        const optionData = allScripts?.data?.[lastIndex1]?.CombineOption;
      ...
      case "Pattern":
      case "Pattern Script":
        const patternData = allScripts?.data?.[lastIndex1]?.CombinePattern;
      ...
      case "GoldenStrategy":
        // You can add a check here if needed later
        const goldenData = allScripts2?.data?.[lastIndex2]?.CombineGolden;
        if (goldenData?.length >= 1) {
      ...
      default:
        const defaultData = allScripts?.data?.[lastIndex1]?.CombineOption;
