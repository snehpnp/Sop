...
const Adduser = () => {
    ...
    const [GetAllPlans, setAllPlans] = useState({ LivePlanName: [], DemoPlanName: [], data: [], SOPDEMO: [], SOPLive: [], ChartDEMO: [], ChartLive: [] });
        ...
        await Get_All_Plans()
            .then((response) => {
                ...
                if (response.Status) {
                    ...
                    const sopDemo = response.Admin.filter((item) => item.PlanType === 'Demo');
                    const sopLive = response.Admin.filter((item) => item.PlanType === 'Live');

                    const chartDemo = response.Charting.filter((item) => item.PlanType === 'Demo');
                    const chartLive = response.Charting.filter((item) => item.PlanType === 'Live');
    ...
    const formik = useFormik({
        ...
        validate: (values) => {
            ...
            if ((values.Select_License === '1' || values.Select_License === '2') && !values.planname && !values.planname2) {
                errors.planname = '❗ Please select at least one plan from SOP or Charting.';
                errors.planname2 = '❗ Please select at least one plan from SOP or Charting.';
            }
            return errors;
        },
        onSubmit: async (values) => {
            ...
            const FilterPlanAmount2 = GetAllPlans.LivePlanName.filter((item) =>
                (item.Planname || item.PlanName) === formik.values.planname2)
            ...
            if ((FilterPlanAmount.length > 0 || FilterPlanAmount2.length > 0) && (totalPlanAmount > values.ClientAmmount)) {
                Swal.fire({
                    // background: "#1a1e23 ",
                    backdrop: "#121010ba",
    ...
    useEffect(() => {
        if (formik.values.Select_License === '1' || formik.values.Select_License === '2') {
            setSelectedSOP(null);
                        ...
                        <AddForm
                            ...
                            additional_field={
                                <>
                                    ...
                                    {
                                        (formik.values.Select_License === "1" || formik.values.Select_License === "2") &&

                                        <div className='d-flex justify-content-between align-items-center gap-3 '>

                                            <div className="col-lg-6 dropdownuser">
                                                <label className='card-text-Color'>{formik.values.Select_License === "1" ? "SOP Demo Plans" : formik.values.Select_License === "2" ? "SOP Live Plans" : "Plans"}</label>
                                                ...
                                                <Select
                                                    ...
                                                    onChange={(selected) => {
                                                        ...
                                                        formik.setFieldValue('planname', selected?.value || "");

                                                        if (!selected && !selectedCharting) {
                                                            setPlanSelectError("Please select at least one plan from SOP or Charting.");
                                                        } else {
                                                            setPlanSelectError("");
                                                        }
                                                    }}
                                            ...
                                            <div className="col-lg-6 dropdownuser">
                                                <label className='card-text-Color'>{formik.values.Select_License === "1" ? "Charting Demo Plans" : formik.values.Select_License === "2" ? "Charting Live Plans" : "Plans"}</label>
                                                ...
                                                <Select
                                                    ...
                                                    onChange={(selected) => {
                                                        ...
                                                        // Update groupName as the combination of both
                                                        formik.setFieldValue('planname2', selected?.value || "");
                                    ...
                                    {planSelectError && (
                                        <div style={{ color: 'red', marginTop: 8, fontWeight: 600, fontSize: 15 }}>{planSelectError}</div>
                                    )}
